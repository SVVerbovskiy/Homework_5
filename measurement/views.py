...
class CreateAPIView(ListAPIView):
    """Создать датчика. Указывается название и описание датчика"""
...
class ListView(ListAPIView):
    """Получить список датчиковю. Выдается список с краткой информацией по датчикам"""
    queryset = Sensor.objects.all()
...
class RetrieveUpdateAPIView(RetrieveAPIView):
    ...
    def patch(self, request, pk):
        """Изменить датчик. Указывается название или лписание"""
...
class ListCreateAPIView(ListAPIView):
    """Добавить измерение. Указывается ID атчика и температура"""
